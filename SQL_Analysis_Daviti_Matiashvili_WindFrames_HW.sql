WITH yearly_sales_data AS (
    ...
    JOIN sh.countries ca ON cu.country_id = ca.country_id
    WHERE
        t.calendar_year BETWEEN 1998 AND 2001  
        AND ca.country_region IN ('Americas', 'Asia', 'Europe')  
    GROUP BY
        t.calendar_year, ca.country_region, c.channel_desc
),
...
sales_percentages AS (
    ...
    JOIN total_sales_by_region tsr ON ys.calendar_year = tsr.calendar_year
...
CumulativeSales AS (
    SELECT ds.*,SUM(ds.sales) OVER (
    ORDER BY ds.time_id ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cum_sum
    FROM DailySales ds),
MovingAverages AS (
    SELECT cs.calendar_week_number,cs.time_id,cs.day_name,cs.sales,cs.cum_sum,
    CASE WHEN LOWER(cs.day_name) = 'monday'
         THEN AVG(cs.sales) OVER (ORDER BY cs.time_id ROWS BETWEEN 2 PRECEDING AND 1 FOLLOWING)
         WHEN LOWER(cs.day_name) = 'friday' 
         THEN AVG(cs.sales) OVER (ORDER BY cs.time_id ROWS BETWEEN 1 PRECEDING AND 2 FOLLOWING)
         ELSE AVG(cs.sales) OVER (ORDER BY cs.time_id ROWS BETWEEN 1 PRECEDING AND 1 FOLLOWING)
        END AS centered_3_day_avg
    FROM CumulativeSales cs)

SELECT calendar_week_number,time_id,day_name,
