...
WITH DailySales AS (
   SELECT 
    ...
    WHERE EXTRACT(YEAR FROM t.time_id) = 1999 
        AND (t.calendar_week_number = 48 AND LOWER(t.day_name) IN ('saturday', 'sunday')
             OR t.calendar_week_number BETWEEN 49 AND 51
             OR t.calendar_week_number = 52 AND LOWER(t.day_name) IN ('monday'))
